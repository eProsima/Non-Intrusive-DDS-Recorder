...
/*
 * The recorder parses only the topic name, the type name and the GUID out of the
...
struct McapRecorder::Impl
{
    ...
    /// An endpoint seen in discovery: the GUID identifying it and the topic it belongs to.
    ...
    const DiscoveredEndpoint* find_endpoint(
            unsigned int hostId,
            unsigned int appId,
            unsigned int instanceId,
            unsigned int entityId);
...
const McapRecorder::Impl::DiscoveredEndpoint* McapRecorder::Impl::find_endpoint(
        unsigned int hostId,
        unsigned int appId,
        unsigned int instanceId,
        unsigned int entityId)
{
    for (const auto& entity : endpoints_)
    {
        if (entity.hostId == hostId && entity.appId == appId
                && entity.instanceId == instanceId && entity.entityId == entityId)
        {
            return &entity;
        }
    }
...
McapRecorder::~McapRecorder()
{
    if (nullptr != impl_)
    {
        ...
        // endpoints_ holds its records by value, so deleting the Impl releases them.
        delete impl_;
    }
}
        ...
        std::string& typeName)
{
    ...
    {
        impl_->endpoints_.push_back({hostId, appId, instanceId, entityId, topicName, typeName});
    }
        ...
        unsigned int serializedDataLen)
{
     ...
     * attributed to it.
     */
    const McapRecorder::Impl::DiscoveredEndpoint* endpoint =
            impl_->find_endpoint(hostId, appId, instanceId, writerId);
    ...
    auto topic_it = impl_->topics_.find(endpoint->topic_name);
