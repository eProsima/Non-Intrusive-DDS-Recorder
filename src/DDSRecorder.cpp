...
DDSRecorder::DDSRecorder(eProsimaLog &log, string &database, int tcMaxSize) : m_log(log), m_databaseH(NULL),
		m_typecodeDB(NULL), m_entitiesDB(NULL), m_tcMaxSize(tcMaxSize), UTCprovider(NULL)
{
		...
		const char *serializedData, unsigned int serializedDataLen)
{
	...
	if(serializedData != NULL)
	{
		...
		{
			// Add typecode.
			if(m_typecodeDB == NULL || m_typecodeDB->addTypecode(pubtopic.topic_name, pubtopic.type_name,typeCode) == false)
			{
				if(sentTypeCode)
					delete typeCode;
			}
		...
		{
			logInfo(m_log, "the datawriter of topic %s doesn't send the typecode, and the user didn't provide it either", pubtopic.topic_name.c_str());
		...
		const char *serializedData, unsigned int serializedDataLen)
{
	...
	if(serializedData != NULL)
	{
		...
		{
			// Add typecode.
			if(m_typecodeDB == NULL || m_typecodeDB->addTypecode(subtopic.topic_name, subtopic.type_name,typeCode) == false)
			{
				if(sentTypeCode)
					delete typeCode;
			}
		...
		{
			logInfo(m_log, "the datareader of topic %s doesn't send the typecode, and the user didn't provide it either", subtopic.topic_name.c_str());
