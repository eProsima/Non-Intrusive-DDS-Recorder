...
def get_version_from_cmakelists(cmakelists):
    ...
    :raises RuntimeError: If the file declares no project version.
    """
    with open(cmakelists, "r") as f:
        content = f.read()

    match = re.search(
        r"project\s*\(\s*\S+\s+VERSION\s+([0-9]+)(?:\.([0-9]+))?(?:\.([0-9]+))?",
        content,
        re.IGNORECASE,
    )
    if match is None:
        raise RuntimeError(
            "Could not find a project() version in {}".format(cmakelists)
        )
...
def retrieve_sidebar_html(root_dir):
    url = (
        "https://raw.githubusercontent.com/eProsima/all-docs/master/source/_templates/sidebar/"
...
def retrieve_sidebar_image(root_dir):
    url_img = (
        "https://raw.githubusercontent.com/eProsima/all-docs/master/source/_static/"
...
def download_css(html_css_dir):
    ...
    """
    url = (
        "https://raw.githubusercontent.com/eProsima/all-docs/master/source/_static/css/"
        "eprosima-furo.css"
    )
    try:
        req = requests.get(url, allow_redirects=True, timeout=10)
...
versions = get_version_from_cmakelists("{}/../../CMakeLists.txt".format(script_path))
...
# of the sidebar.
#
html_logo = "rst/_static/dds-recorder-logo.svg"
