...
.. code-block:: bash
    ...
    ni_ddsrecorder -mcap session.mcap capture.pcap
...
.. note::
    ...
    that differs from the one a database recording of the same capture produces, even though every
    other field matches.
...
``Messages`` table of a database recording, which makes the two outputs directly comparable.
